...
def request_artist_info(artist_name):
    ...
    for ly in song:
        lis.append(ly.get_text(separator=" ").strip())
    # remove identifiers like chorus, verse, etc
    return lis

artistName = input("enter artist: ")
song_list = request_artist_info(artistName)

for song in song_list:
    url = input_model.request_artist_song_url(artistName,song)
    print(url)
    input_model.check_url_exist(url)
...
#
#
# for tup in artist_songs_tuplis:
#     song_name_str = tup[2]
#     song_url = tup[1]
#     print(song_url)
